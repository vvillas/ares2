...
*/
DROP SCHEMA IF EXISTS ares;
CREATE SCHEMA ares;

DROP TABLE IF EXISTS ares.tbl_group_user;
DROP TABLE IF EXISTS ares.tbl_command_node;
DROP TABLE IF EXISTS ares.tbl_user;
DROP TABLE IF EXISTS ares.tbl_person;
DROP TABLE IF EXISTS ares.tbl_group;
DROP TABLE IF EXISTS ares.tbl_node;
DROP TABLE IF EXISTS ares.tbl_command;
DROP TABLE IF EXISTS ares.tbl_session;

CREATE TABLE ares.tbl_person (
...
CREATE TABLE ares.tbl_user (
    ...
    FOREIGN KEY (per_in_id) REFERENCES ares.tbl_person (per_in_id) ON DELETE RESTRICT ON UPDATE CASCADE
);


CREATE TABLE ares.tbl_group (
...
CREATE TABLE ares.tbl_group_user (
    ...
    FOREIGN KEY (grp_in_id) REFERENCES ares.tbl_group (grp_in_id) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (usr_in_id) REFERENCES ares.tbl_user (usr_in_id) ON DELETE RESTRICT ON UPDATE CASCADE
);


CREATE TABLE ares.tbl_node (
...
CREATE TABLE ares.tbl_command_node (
    ...
    FOREIGN KEY (com_in_id) REFERENCES ares.tbl_command (com_in_id) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (nod_in_id) REFERENCES ares.tbl_node (nod_in_id) ON DELETE RESTRICT ON UPDATE CASCADE
);


CREATE TABLE ares.tbl_session(
...
INSERT INTO ares.tbl_person (per_st_name, per_st_shortname, per_st_email)
    VALUES ( 'Administrador do Sistema', 'Admin', 'admin@ares.net');
INSERT INTO ares.tbl_user (usr_st_login, usr_st_password, per_in_id, usr_st_password_lembrete)
    VALUES ( 'admin', md5('admin123'), LAST_INSERT_ID(), 'admin123');
